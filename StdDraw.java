...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    *************************************************************************/
    ...
    public static void paragraph(double x, double y, double x2, double y2, Deque<String> s) {
        offscreen.setFont(font);
        FontMetrics metrics = offscreen.getFontMetrics();
        double xs = scaleX(x);
        double ys = scaleY(y);
        double xs2 = scaleX(x2);
        double ys2 = scaleY(y2);
        double currentX = xs;
        double currentY = ys;
        int lines = Math.abs((int) ((ys - ys2)/metrics.getHeight()));
        int count = 0;
        int[] lineCount = new int[lines];
        int cur = 0;
        for(String i: s){
            int height = metrics.getHeight();
            int hs = metrics.getDescent();
            int ws = metrics.stringWidth(i + " ");
            if(currentX + ws >= xs2){
                currentX = xs;
                currentY += height;
                cur++;
            ...
            if(cur >= lines) {
                count += lineCount[0];
                for(int j = 0; j < lineCount.length - 1; j++){
        ...
        for (String i: s){
            if(count > 0) count--;
                ...
                offscreen.drawString(i + " ", (float) (currentX), (float) (currentY + hs));
                currentX += ws;
            }
    ...
    public static void redText(double x, double y, String s) {
        offscreen.setFont(font);
        FontMetrics metrics = offscreen.getFontMetrics();
        double xs = scaleX(x);
        double ys = scaleY(y);
        ...
        int firstSize = metrics.stringWidth(first);
        int middleSize = metrics.stringWidth(middle);
        int lastSize = metrics.stringWidth(last);
        int ws = metrics.stringWidth(s);
        int hs = metrics.getDescent();
        Color temp = getPenColor();
        //StdOut.println(first + " " + middle + " " + last);
        offscreen.drawString(first, (float) (xs - firstSize - middleSize/2.0), (float) (ys + hs));
        setPenColor(StdDraw.RED);
        offscreen.drawString(middle, (float) (xs - middleSize/2.0), (float) (ys + hs));
        setPenColor(temp);
        offscreen.drawString(last, (float) (xs + middleSize/2.0), (float) (ys + hs));
        draw();
    }
